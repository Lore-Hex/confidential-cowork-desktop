...
let app
try {
  const executablePath = process.env.COWORK_SMOKE_EXECUTABLE
  const args = executablePath ? [] : [process.env.COWORK_SMOKE_APP_PATH || '.']
  app = await electron.launch({ executablePath, args: [...args, `--user-data-dir=${data}`], env: { ...process.env, PI_DESKTOP_USER_DATA_DIR: data }, timeout: 30000 })
  const page = await app.firstWindow()
  await page.getByRole('heading', { name: 'Your confidential workspace' }).waitFor()
  assert.equal(await page.getByRole('button', { name: 'Sign in with TrustedRouter', exact: true }).isVisible(), true)
  assert.equal(await page.locator('#trustedrouter-key').count(), 0)
  await page.getByRole('button', { name: 'Use an API key instead', exact: true }).click()
  await app.evaluate(({ shell }) => {
    globalThis.coworkSmokeOpenedUrls = []
    shell.openExternal = async (url) => { globalThis.coworkSmokeOpenedUrls.push(url) }
  })
  await page.getByRole('button', { name: 'Sign in with TrustedRouter', exact: true }).click()
  await page.getByRole('button', { name: 'Cancel sign-in', exact: true }).waitFor()
  await page.waitForTimeout(100)
  const authUrls = await app.evaluate(() => globalThis.coworkSmokeOpenedUrls)
  assert.equal(authUrls.length, 1)
  const auth = new URL(authUrls[0])
  assert.equal(auth.origin + auth.pathname, 'https://trustedrouter.com/auth')
  assert.equal(auth.searchParams.get('code_challenge_method'), 'S256')
  assert.equal(auth.searchParams.has('code_verifier'), false)
  await page.getByRole('button', { name: 'Cancel sign-in', exact: true }).click()
  await page.getByRole('button', { name: 'Sign in with TrustedRouter', exact: true }).waitFor()
  await app.evaluate(() => { globalThis.coworkSmokeOpenedUrls = [] })
