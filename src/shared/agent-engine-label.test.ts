...
/**
 * The status bar named "TRCC" while OMP was the configured engine, because the
...
test('each engine has its own display name', () => {
  assert.equal(agentEngineLabel('pi'), 'TRCC')
...
test('the fallback is a real engine name, not a placeholder', () => {
  ...
  assert.equal(DEFAULT_AGENT_ENGINE_LABEL, 'TRCC')
