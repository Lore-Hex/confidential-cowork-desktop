...
test('quoteDesktopExec wraps in double quotes and escapes special chars', () => {
  assert.equal(quoteDesktopExec('/opt/TR Confidential Cowork/app'), '"/opt/TR Confidential Cowork/app"')
  assert.equal(quoteDesktopExec('/a\\b'), '"/a\\\\b"')
  assert.equal(quoteDesktopExec('/a"b'), '"/a\\"b"')
})

test('buildLinuxDesktopEntry produces a valid, complete desktop entry', () => {
  const entry = buildLinuxDesktopEntry({ exec: '/opt/TR Confidential Cowork.AppImage', icon: '/opt/icon.png' })
  const lines = entry.split('\n')

  assert.equal(lines[0], '[Desktop Entry]')
  assert.ok(lines.includes('Type=Application'))
  assert.ok(lines.includes('Name=TR Confidential Cowork'))
  assert.ok(lines.includes('Exec="/opt/TR Confidential Cowork.AppImage"'))
