...
import { app, BrowserWindow, dialog, nativeImage, nativeTheme, protocol, session, shell } from "electron"
import { confidentialRendererRequest } from "./confidential-network"
...
export function createMainWindow(id: string = randomUUID()) {
  ...
  win.webContents.session.webRequest.onBeforeRequest((details, callback) => {
    callback({
      cancel: !confidentialRendererRequest(details.url, app.isPackaged ? undefined : process.env.ELECTRON_RENDERER_URL),
    })
  })
...
export function registerRendererProtocol() {
  ...
  protocol.handle(rendererProtocol, async (request) => {
    ...
    try {
      ...
      const response = await session.fromPartition("trusted-cowork-assets").fetch(pathToFileURL(file).toString(), {
