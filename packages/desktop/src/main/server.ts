...
import { confidentialEnvironment } from "./confidential-runtime"
...
export async function spawnLocalServer(
  ...
  const child = utilityProcess.fork(sidecar, [], {
    cwd: process.cwd(),
    env: { ...createSidecarEnv(), ...confidentialEnvironment(options.userDataPath, options.confidentialConfig) },
...
function createSidecarEnv(): Record<string, string> {
  ...
  const env = Object.fromEntries(
    Object.entries(process.env).flatMap(([key, value]) =>
      value === undefined || (!allowed.has(key) && !key.startsWith("LC_")) ? [] : [[key, String(value)]],
    ),
  )
  delete env.DEBUG
