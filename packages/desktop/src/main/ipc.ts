...
type Deps = {
  ...
  accountSignIn: () => Promise<void>
  accountCancel: () => Promise<void>
  accountSaveKey: (key: unknown) => Promise<void>
...
export function registerIpcHandlers(deps: Deps) {
  const accountHandlers = {
    "account-status": deps.accountStatus, "account-sign-in": deps.accountSignIn,
    "account-cancel": deps.accountCancel, "account-save-key": deps.accountSaveKey, "account-sign-out": deps.accountSignOut,
  }
  for (const [name, handler] of Object.entries(accountHandlers)) {
    ipcMain.handle(name, async (event: IpcMainInvokeEvent, value: unknown) => {
      const win = BrowserWindow.fromWebContents(event.sender)
      if (!win || event.senderFrame !== event.sender.mainFrame) throw new Error("Unauthorized account operation")
