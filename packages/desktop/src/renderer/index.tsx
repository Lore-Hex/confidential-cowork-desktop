...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  const [account, { refetch }] = createResource(() => window.api.accountStatus())
    ...
    <PlatformProvider value={platform}>
      ...
      >
        <Show when={!account.loading} fallback={<LoadingSplash />}>
          <Show when={account()} fallback={<AccountSignIn onSignedIn={() => void refetch()} />}>
            <div class="trcc-workspace">
              <header class="trcc-account-bar">
                <span>Trusted Cowork</span>
                <Button variant="ghost" size="small" onClick={() => void window.api.accountSignOut()}>Sign out</Button>
              </header>
              <div class="trcc-workspace-body"><App /></div>
            </div>
          </Show>
        </Show>
      </AppBaseProviders>
